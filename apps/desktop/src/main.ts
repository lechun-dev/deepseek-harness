...
async function main(): Promise<void> {
  ...
  let locale = resolveDesktopLocale(resolveDesktopLanguage(undefined, systemLanguages, app.getLocale()))
  ...
  const updates = new DesktopUpdateCoordinator(
    ...
    async () => {
      ...
      if (host === undefined || host.attached) throw new DesktopUpdatePreparationError('tasks-unavailable', locale.messages.updateTasksUnavailable)
  ...
  ipcMain.on(DESKTOP_IPC.localeChanged, (event, next: unknown) => {
    ...
    const current = resolveDesktopLocale(resolveDesktopLanguage(next, systemLanguages, app.getLocale()))
  ...
  const openInitialWindow = async (): Promise<void> => {
    ...
    locale = resolveDesktopLocale(resolveDesktopLanguage(state.localePreference ?? undefined, systemLanguages, app.getLocale()))
    windowsLanguage = locale.id
    applyApplicationMenu()
