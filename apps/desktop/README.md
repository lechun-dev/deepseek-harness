...
The desktop application is an Electron shell around the complete dsh Web application. An Electron RunAsNode child starts the shared profile runner, and Electron immediately loads the packaged Web entry at `dsh-app://app/`. Its shared loading page waits for Host boot injections, then starts the client without navigating to another document. Electron forwards application HTTP requests to the authenticated Web Host; WebSocket streams connect to that Host with credentials attached only for the owned application window. Node IPC carries boot injections, readiness, and shutdown. Desktop and Web default to the same port `3080`, which a `webserver.config.port` patch can override: this Harness home serves one Web runtime, so the child adopts the service another surface published there instead of binding a second socket, and the desktop window shares its sessions and streams with a running `dsh web`. When that port is already held and this home published no adoptable service, the child binds an OS-assigned loopback port instead of failing startup. The application menu opens the serving service in the default browser through its authenticated URL, which is the exchange a browser needs once to store its own cookie; the clean loopback address then answers on its own.

The first packaged launch asks once whether to install the machine's `dsh` command. The launcher it writes onto `PATH` — preferring `/opt/homebrew/bin` and then `/usr/local/bin` on macOS, where an administrator-owned directory uses the operating system's authorization prompt, and the user's `WindowsApps` alias directory on Windows, which is user-owned and needs no administrator — runs the bundled runtime through the application's own Electron binary in Node mode. That is what lets a PATH-only consumer such as MissionOS use the same runtime the desktop window serves. The application menu repeats the offer and removes the launcher, restoring the `dsh` it replaced: a recorded symlink target, or a regular file moved to `<path>.<version>.bak`. The launcher records absolute application paths, so moving the application requires reinstalling it from the menu. On Windows that launcher is a small executable carried by this build — a program that is not a shell cannot start a .cmd — and it reads a sibling `dsh-shim.json` for the application and entry it targets. The result dialog also reports whether the Multica bridge profile answered (`--profile multica --probe`), which is the gate another runtime reads before registering this machine, and names any earlier `dsh` on `PATH` that still takes precedence. The first-launch question is recorded only after a decline or a completed attempt, so a failed installation is offered again on the next launch.
