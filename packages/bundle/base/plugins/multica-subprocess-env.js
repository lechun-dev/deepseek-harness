 ...
 */
export const FORWARDED_ENV_NAMES = ['MULTICA_TOKEN'];
/**
 ...
 */
export function forwardedEnv(source = process.env) {
    const forwarded = {};
 ...
 */
export function withForwardedEnv(spec, source = process.env) {
...
export function apply(ctx) {
    ...
    const decoratedSpawn = (spec) => spawn.call(runtime, withForwardedEnv(spec));
    const decoratedSpawnTerminal = (spec) => spawnTerminal.call(runtime, withForwardedEnv(spec));
    ctx.effect(() => {
        runtime.spawn = decoratedSpawn;
        runtime.spawnTerminal = decoratedSpawnTerminal;
        return () => {
            if (runtime.spawn === decoratedSpawn)
                runtime.spawn = spawn;
            if (runtime.spawnTerminal === decoratedSpawnTerminal)
                runtime.spawnTerminal = spawnTerminal;
        };
